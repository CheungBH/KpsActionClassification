...
def main():
    ...
    if save_model is True:
        ...
        AB.fit(train_feature, train_target)
        predict_results_train = AB.predict(train_feature)
        predict_results_test = AB.predict(test_feature)
        dump(AB, f"{args.output_folder}/model.joblib")
        ...
        result_data = ["AdaBoost", train_accuracy, test_accuracy] + [acc for acc in cls_result]
        ...
        for n_estimator in [10, 50, 100, 200]:
            for lr in [0.1, 0.5, 1, 1.5]:
                for algo in ["SAMME.R", "SAMME"]:
                    AB = AdaBoostClassifier(n_estimators=n_estimator, learning_rate=lr, algorithm=algo)
                    AB.fit(train_feature, train_target)
                    predict_results_train = AB.predict(train_feature)
                    predict_results_test = AB.predict(test_feature)

                    train_accuracy = accuracy_score(predict_results_train, train_target)
                    print("\n\nAdaBoost " + f"n_estimators: {n_estimator}, learning_rate: {lr}, algorithm: {algo}")
                    # print("Training accuracy is : ", train_accuracy)
                    test_accuracy = accuracy_score(predict_results_test, test_target)
                    ...
                    result_title = ["algo", 'Train Acc', 'Test Acc'] + ['Class ' + str(i) + ' Acc' for i in range(len(cls_result))]
                    result_data = ["AdaBoost ", train_accuracy, test_accuracy] + [acc for acc in cls_result] + [f"n_estimators: {n_estimator}, learning_rate: {lr}, algorithm: {algo}"]
