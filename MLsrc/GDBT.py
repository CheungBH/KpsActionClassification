...
def load_data(file_path, feature_num):
    ...
    for content in csv_file:
        # content = list(map(float, content[:data_final]))
        if len(content) != 0:
...
def main():
    ...
    parser.add_argument("--save_lowest_acc", type=float, default=90)
    # parser.add_argument("--csv_path", type=str, help="Path to save the results in csv")
    args = parser.parse_args()
    os.makedirs(args.output_folder, exist_ok=True)

    save_model = False
    train_file = f"{args.data_root}/train.csv"
    test_file = f"{args.data_root}/val.csv"
    csv_path = f"{args.output_folder}/results.csv"
                            ...
                            for weight_fraction_leaf in [0, 0.1, 0.3]:
                                for depth in [1, 3, 10]:
                                    for impurity_decrease in [0, 0.5, 1]:
                                        search_idx += 1
                                        gbdt = GradientBoostingClassifier(learning_rate=lr,
                                                                          n_estimators=est,
                                                                          subsample=sub,
