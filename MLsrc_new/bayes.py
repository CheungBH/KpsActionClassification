...
def main():
    ...
    NB = BernoulliNB(**config)
    NB.fit(train_feature, train_target)

    predict_results = NB.predict(test_feature)
    dump(NB, f"{args.output_folder}/model.joblib")
    ...
    result_data = ["Bayes", train_accuracy, test_accuracy] + [acc for acc in cls_result]
